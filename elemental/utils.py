...
def element_downloader(element, local_file=None, return_fmt="array"):
    ...
    Keyword Arguments:
        ...
        return_fmt {str} -- type of the return format. Options: 'array', 'dict' (default: {"array"})
    ...
    if local_file:
        ...
        #     '%s&limits_type=0&low_w=&upp_w=&unit=1&submit=Retrieve+'
        #     'Data&de=0&format=0&line_out=0&en_unit=0&output=0&bibre'
        #     'fs=1&page_size=15&show_obs_wl=1&show_calc_wl=1&unc_out'
        #     '=1&order_out=0&max_low_enrg=&show_av=2&max_upp_enrg=&t'
        #     'sb_value=0&min_str=&A_out=0&intens_out=on&max_str=&all'
        #     'owed_out=1&forbid_out=1&min_accur=&min_intens=&conf_ou'
        #     't=on&term_out=on&enrg_out=on&J_out=on' % element)
        nist_webpage = (
            "https://physics.nist.gov/cgi-bin/ASD/lines1.pl?spectra="
        ...
        # html = urlopen("https://google.com")#.read().decode('utf-8')
    ...
    # pre_string_intensity = r'\<td class\=\"cnt\"\>\&nbsp\;[\W]*'
    # post_string_intensity = r'[\&nbsp\;\<|\<a class]'  # Cut-off at "<".
    # search_string_intensity = r'%s[\(\)]?([\d]+)[\(\)A-Za-z]?%s' % (pre_string_intensity,
    #                                                                 post_string_intensity)
    ...
    retreived_data = []  # List of dictionaries

    # For verifiying that we actually find a spectra with a matching intensity.
    counter = -3
    # For saving if we found an estimated intensity for a given wavelength.
    ...
    for i, l in enumerate(html):
        ...
        match_wl = regex_sequence_wl.findall(l)
    ...
    if return_fmt == "array":
        ...
        raise KeyError("%s not recognized. Use 'array' or 'dict'.")
